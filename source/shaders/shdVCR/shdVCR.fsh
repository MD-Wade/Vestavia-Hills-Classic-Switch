...
float rand(vec2 co){
    return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * 43758.5453) * fIntensity;
}
void main( void )
{
 ...
 // Jitter each line left and right
    ...
    // Either sample the texture2D, or just make the pixel white (to get the staticy-bit at the bottom)
    ...
    if (whiteNoise > 11.5-30.0*samplePosition.y || whiteNoise < 1.5-5.0*samplePosition.y) {
        // Sample the texture2D.
    //samplePosition.y = 1.0-samplePosition.y; //Fix for upside-down texture2D
    texColor = texColor + texture2D(gm_BaseTexture,samplePosition);
    } else {
        // Use white. (I'm adding here so the color noise still applies)
        texColor = texture2D(gm_BaseTexture,samplePosition) + vec4(fIntensity, fIntensity, fIntensity, fIntensity);
    }
