/// @description PlayerStepInputs();
function PlayerStepInputs() {
	inputUp = abs(clamp(gamepad_axis_value(global.GP_ID, gp_axislv), -1, 0));
	inputLeft = abs(clamp(gamepad_axis_value(global.GP_ID, gp_axislh), -1, 0));
	inputDown = abs(clamp(gamepad_axis_value(global.GP_ID, gp_axislv), 0, 1));
